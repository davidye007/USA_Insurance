...
renderHighchart({
  ...
  Base_Data_border<- Base_data %>% filter(InsuranceType == input$insur_type, Year == input$year, State == ifelse(is.null(input$selected_state), "Washington", input$selected_state))
...
```{r}
data = list(
  list(usgeojson$woename == "California")  # Specify California to add a red border
)

observe({
  print(input$selectedState)  # This will print the selected year in the R console
  print(data)
})
